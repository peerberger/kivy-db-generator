...
class MyGrid(Widget):
    ...
    def btn(self):
        ...
        # threading.Thread(target=self.fill_progress_bar, args=[]).start()
        # self.fill_progress_bar()
        # Clock.schedule_once(partial(self.fill_progress_bar), 0)

    # @mainthread
    def fill_progress_bar_thread(self):
        self.ids.bar.value = 0
        while self.ids.bar.value < self.ids.bar.max:
            self.ids.bar.value += 1
            time.sleep(.1)

    def fill_progress_bar_clock(self):
